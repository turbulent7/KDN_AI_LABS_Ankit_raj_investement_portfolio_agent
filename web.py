...
def analyze_portfolio(portfolio, risk_tolerance, exit_period):
    ...
    if sector_counts.get("Technology", 0) > 2:
        st.warning("⚠️ High concentration in a single sector. Consider diversifying.")

    # Recommendations
    recommendations = []
    if risk_tolerance == "Moderate":
        recommendations.append("📌 Reduce exposure to high-volatility stocks.")
        recommendations.append("📌 Consider adding defensive sectors like healthcare or utilities.")
    if exit_period <= 5:
        recommendations.append("📌 Invest in stable dividend-paying stocks to preserve capital.")
    ...
    # Market Trends
    st.subheader("🔹 Market Trends Impacting Portfolio")
    news_results = search_news("Stock market trends")
    for title, link in news_results:
        st.markdown(f"- [{title}]({link})")

# Wait for user to press Analyze button
if st.button("Analyze Portfolio"):
    st.subheader("Enter Your Portfolio")
    portfolio = {}
    num_stocks = st.number_input("How many stocks do you want to add?", min_value=1, max_value=10, value=1)
    for i in range(num_stocks):
        stock = st.text_input(f"Stock Symbol {i+1}", key=f"stock_{i}").upper()
        shares = st.number_input(f"Shares of {stock}", min_value=1, value=1, key=f"shares_{i}")
        if stock:  # Only add non-empty symbols
            portfolio[stock] = shares

    if portfolio:  # Only analyze if the user has entered stocks
        analyze_portfolio(portfolio, risk_tolerance, exit_period)
    else:
        st.warning("Please enter at least one stock before analyzing.")
